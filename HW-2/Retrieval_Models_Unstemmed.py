...
def getCatalog(merge_catalog):
    c_path = "./Unstemmed_CatalogFiles_wo-dfttf/main_catalog.txt"
    c = open(c_path, 'r')
    for c_line in c:
...
def okapiTF(data, query_no):
    ...
    f = open("./okapiTF_unstemmed.txt", "a+")
    rank = 1
...
def unigram_laplace(termVectors, query_no):
    ...
    f = open('./laplace_unstemmed.txt', 'a+')
    rank = 1
...
inv_path = "./Unstemmed_InvFiles_wo-dfttf/main_inv.txt"
