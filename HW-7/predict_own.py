...
file_own = open('./spam_own.txt').read()
spam_own = file_own.split("\n")

vctzr = CountVectorizer(ngram_range=(1, 3), vocabulary=list(set(spam_own)), analyzer="word", min_df=0.001, max_df=0.995)  # ngram_range=(1, 5),
fitted_X_train = vctzr.fit_transform(x_train)
transformed_X_test = vctzr.transform(x_test)
# print(vctzr.vocabulary_)
print(type(vctzr))

# dump_svmlight_file(fitted_X_train, y_train, './dump_own.txt')
